...
PIDController default_pid(1, 0.0, 0.04, 10000, 7.4);
...
HapticState::HapticState(void){
    detent_profile = default_profile;//default_profile;
...
Cust_HapticInterface::Cust_HapticInterface(BLDCMotor* _motor){
    motor = _motor;
    haptic_pid = &default_pid;
    haptic_state = HapticState(default_profile);
};

Cust_HapticInterface::Cust_HapticInterface(BLDCMotor* _motor, PIDController* _pid){
    motor = _motor;
    haptic_pid = _pid;
    haptic_state = HapticState(default_profile);
};
...
*/
void Cust_HapticInterface::correct_pid(void)
{
    bool clipping;
...
*/
void Cust_HapticInterface::offset_detent(void){
...
*/
void Cust_HapticInterface::find_detent(void)
{
    /**
...
*/
void Cust_HapticInterface::detent_handler(void){
    ...
    if(haptic_state.last_attract_angle > haptic_state.attract_angle){
        if(motor->sensor_direction != Direction::CCW){
            ...
            if(haptic_state.current_pos > haptic_state.detent_profile.start_pos){
                ...
                haptic_state.atNegLimit = false;
                haptic_state.atPosLimit = false;
...
float Cust_HapticInterface::haptic_target(void)
{
    ...
    float motor_shaft_angle = motor->shaft_angle;
    float error = haptic_state.last_attract_angle - motor_shaft_angle;
    Serial.println(motor_shaft_angle);
    ...
    if(fabsf(motor->shaft_velocity) > 30) {
        ...
        error = CLAMP(
            error,
            haptic_state.atPosLimit ? 0 : -detent_width,
            haptic_state.atNegLimit ? 0 : detent_width
        );
        motor->move(default_pid(error));
    }

    haptic_pid->operator()(error);
    return motor_shaft_angle;
}

/**
 * Internal detent update handler. Don't use this, usually.
 */
void Cust_HapticInterface::HapticEventCallback(HapticEvt event){
...
*/
void Cust_HapticInterface::UserHapticEventCallback(HapticEvt event, float currentAngle, uint16_t currentPos){
    /**
